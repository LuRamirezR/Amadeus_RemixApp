...
export default function Graphics({ data }: { data: any[] }) {
  ...
  useEffect(() => {
    ...
    Object.keys(groupedData).forEach((question) => {
      formattedData[question] = Object.keys(groupedData[question]).map((key) => ({
        name: key,
  ...
  if (Object.keys(chartsData).length === 0) return <p>⏳ Cargando gráficos...</p>;
      ...
      {Object.keys(chartsData).map((question, index) => (
        <div key={index} className="chart-container">
          ...
          <PieChart width={400} height={400}>
            <Pie data={chartsData[question]} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100}>
